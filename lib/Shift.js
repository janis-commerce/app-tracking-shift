	...
	async open({warehouseId = ''} = {}) {
		try {
			...
			if (!userId || isEmptyObject(remoteShift)) {
				...
				this.id = shiftId;
				this.data = await this.getUserOpenShift({id: shiftId});
			...
			// REUSE: same shift, falls through to the return. Local storage already
			// holds this shift's id, status and data, so nothing needs to be updated.
