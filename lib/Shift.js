...
class Shift {
	...
	async open({warehouseId = ''} = {}) {
		try {
			...
			// partial write can leave the id without its status or data. Each one is
			// only set when there is nothing stored, keeping what the app already knows:
			// a local 'paused' is not reflected by the staff MS and overwriting it would
			// resume a paused shift, and the stored data may hold a dateToClose already
			// extended or a warehouseId the staff MS does not reflect yet.
			if (!this.status) this._changeStatus(remoteShift.status);

			if (isEmptyObject(this.data || {})) this.data = remoteShift;
