...
class Shift {
	...
	async open({warehouseId = ''} = {}) {
		try {
			...
			// partial write can leave the id without its status or data. The data is
			// always replaced with the remote shift, the source of truth, which covers
			// it being absent, empty or incomplete. The status is only set when missing,
			// because a local 'paused' is not reflected by the staff MS and overwriting
			// it would resume a paused shift.
