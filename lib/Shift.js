...
class Shift {
	...
	/**
	 * Pause the shift by updating the status in local storage.
	 * @returns {Array} [null, null] => if the shift was paused successfully, [null, error] => if an error occurred
	 */
	...
	/**
	 * Resume the shift by updating the status in local storage.
	 * @returns {Array} [null, null] => if the shift was resumed successfully, [null, error] => if an error occurred
	 */
	...
	async openWorkLog(params = {}) {
		try {
			...
			const {referenceId, name, type, suggestedTime = 0} = params;
