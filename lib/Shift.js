import {isArray, isEmptyArray, isEmptyObject, isNumber} from '@janiscommerce/apps-helpers';
...
import CustomError, {INTERNAL_CODE} from './CustomError';
import {
	isShiftClosedError,
	isInternetReachable,
	isValidDate,
...
class Shift {
	...
	set id(id) {
		Storage.set(SHIFT_ID, id, {expireWithVersion: true});
	}
	...
	set status(status) {
		Storage.set(SHIFT_STATUS, status, {expireWithVersion: true});
	}
	...
	set data(data) {
		Storage.set(SHIFT_DATA, data, {expireWithVersion: true});
	}
	...
	async open({warehouseId = ''} = {}) {
		try {
			...
			const remoteShift = userId ? await this.getUserOpenShift({id: this.id, userId}) : {};

			// NEW: no userId or no remote open shift for the user
			if (!userId || isEmptyObject(remoteShift)) {
				await this.deleteShiftRegisters();
				const {result: shift} = await StaffService.openShift({warehouseId});
				const {id: shiftId = ''} = shift || {};
				const openedShift = await this.getUserOpenShift({id: shiftId});

				// The id is persisted last, acting as the commit marker of the three keys:
				// everything else gates on it, so an interruption before this point leaves
				// no shift at all (recoverable through ADOPT on the next open) instead of
				// an id without its status and data.
				this._changeStatus('opened');
				this.data = openedShift;
				this.id = shiftId;

				return shiftId;
			}

			// ADOPT: remote shift open with a different id than the local one
			if (this.id !== remoteShift.id) {
				this.deleteCurrentWorkLog();
				ShiftInactivity.reset();
				this._changeStatus(remoteShift.status);
				this.data = remoteShift;
				this.id = remoteShift.id;
			}

			// REUSE: same shift. The storage is key-value without transactions, so a
			// partial write can leave the id without its status or data. Only what is
			// missing gets rehydrated, to preserve the local dateToClose extension and
			// a warehouseId the staff MS may not reflect yet. The data is checked
			// through dateMaxToClose because a spread over an absent data (update,
			// finish, _extendShiftClosingDate) leaves an object holding a single key,
			// which is neither absent nor empty but is still unusable.
			if (!this.status) this._changeStatus(remoteShift.status);

			if (!this.data?.dateMaxToClose) this.data = remoteShift;

			return remoteShift.id;
		} catch (error) {
	...
	async update({warehouseId = ''} = {}) {
		try {
			...
			const {result: shift} = await this._withReopenRetry(() =>
				StaffService.updateShift({warehouseId})
			);
			const {id: shiftId = ''} = shift || {};
	...
	async finish({date} = {}) {
		try {
			...
			const {result: shift} = await this._withReopenRetry(() => StaffService.closeShift({endDate}));
			const {id: shiftId = ''} = shift || {};
	...
	async openWorkLog(workLog = {}) {
		...
		try {
			...
			if (!ShiftWorklogs.isValidWorkLog(workLog)) {
				throw new CustomError('must provide a valid activity to open a work log', INTERNAL_CODE);
			}
			...
			openWorkLog.shiftId = this.id;
			this.setCurrentWorkLog(openWorkLog);

			// Activity is persisted BEFORE adjusting the status: an interruption leaves 'opened'
			// with activity (recoverable), never 'paused' without activity (blocking).
			// The status always reflects the activity: pausing -> paused, otherwise -> opened.
			const requirePauseShift = this._hasToPause(openWorkLog.referenceId);

			this._changeStatus(requirePauseShift ? 'paused' : 'opened');
			...
			await this._withReopenRetry(
				...
				{updateStatusAfterReOpen: !requirePauseShift}
			);

			return openWorkLog.id;
		} catch (error) {
			if (!CustomError.isInternalError(error)) {
				this._saveOffLineWorkLogs([previousWorkLog, openWorkLog]);
			}
	...
	async finishWorkLog(workLog = {}) {
		...
		try {
			...
			if (!ShiftWorklogs.isValidWorkLog(workLog)) {
				throw new CustomError('must provide a valid activity to close a work log', INTERNAL_CODE);
			}
			...
			if (!ShiftWorklogs.isValidWorkLog(currentWorkLog)) {
				throw new CustomError('There is no active worklog to close', INTERNAL_CODE);
			}
			...
			// The activity belongs to the shift it was opened in, not to whatever
			// shift is current when the offline save runs.
			finishWorkLog.shiftId = currentWorkLog?.shiftId;
			...
			await this._withReopenRetry(async () => {
				if (this.hasPendingData) {
					const offLineWorkLogs = this._getOffLineWorkLogs();
	...
	setCurrentWorkLog(workLog = {}) {
		const {suggestedTime = 0, id, startDate, shiftId} = workLog;
		const suggestedFinishDate = new Date(startDate).getTime() + suggestedTime * 60 * 1000;
		const currentWorkLog = {
			...workLog,
			shiftId: shiftId ?? this.id,
			suggestedFinishDate: new Date(suggestedFinishDate).toISOString(),
		};
		Storage.set(CURRENT_WORKLOG_ID, id, {expireWithVersion: true});
		Storage.set(CURRENT_WORKLOG_DATA, currentWorkLog, {expireWithVersion: true});
	}
	...
	async sendPendingWorkLogs() {
		try {
			...
			await this._withReopenRetry(() => ShiftWorklogs.batch(formattedWorkLogs));
			OfflineData.deleteAll();
			return null;
		} catch (error) {
			const parsedError = errorParser(error);
	...
	async getWorkLogs(shiftId) {
		try {
			...
			if (!userShiftId) throw new CustomError('Shift ID not found', INTERNAL_CODE);
	...
	/**
	 * Refresh the current work log from the staff MS: fetch the shift work logs,
	 * detect the one in progress, persist it and pause the shift when it applies.
	 * @throws {Error} error
	 * @returns {Promise<void>}
	 */

	async refreshWorkLogs() {
		const workLogs = await this.getWorkLogs(this.id);
		const [openWorkLogs = []] = Formatter.splitWorkLogsByStatus(workLogs);
		const [currentWorkLog = {}] = openWorkLogs;

		if (ShiftWorklogs.isValidWorkLog(currentWorkLog)) {
			this.setCurrentWorkLog(currentWorkLog);
		}

		if (ShiftWorklogs.isPausingWorkLog(currentWorkLog)) {
	 ...
	 * @param {Object} params
	 * @param {boolean} params.updateStatusAfterReOpen => Whether to update the shift status to opened
	 * @throws {Error} error
	 * @returns {Promise<null>} null
	 */

	async reOpen({updateStatusAfterReOpen = true} = {}) {
		try {
			...
			if (this.isExpired()) {
				...
				throw new CustomError('The deadline for ending the shift has been exceeded', INTERNAL_CODE);
			}

			await StaffService.openShift({warehouseId: this.data?.warehouseId});
			if (updateStatusAfterReOpen) this._changeStatus('opened');
	 ...
	 * shift data, current work log, inactivity timer expiry and offline buffer.
	 * @throws {Error} error
	...
	/**
	 * @private
	 * Attempt callback(); if it fails with a closed shift error, reopen and retry once.
	 * @param {() => Promise<any>} callback => The callback to execute
	 * @param {Object} reopenOptions => The options to pass to the reOpen method
	 * @returns {Promise<any>} The result of the callback
	 */

	async _withReopenRetry(callback, reopenOptions = {}) {
		try {
			return await callback();
		} catch (error) {
			if (!isShiftClosedError(error)) throw error;
			await this.reOpen(reopenOptions);
			return callback();
		}
	}
	...
	_saveOffLineWorkLogs(workLogs) {
		...
		workLogs.forEach((workLog) => {
			const {id, referenceId, startDate, endDate, shiftId} = workLog || {};
	...
	_getOffLineWorkLogs() {
		const offlineWorkLogs = OfflineData.get();
		const validWorkLogs = ShiftWorklogs.filterShiftWorkLogs({
			workLogs: offlineWorkLogs,
			shiftId: this.id,
			shiftStartDate: this.data?.startDate,
		});

		// Length differs <=> there are records from another shift or dated before this
		// one started (orphans): compact the buffer keeping only the current shift's
		// valid records to avoid stalling the drain.
		if (offlineWorkLogs.length !== validWorkLogs.length) {
			OfflineData.replaceAll(validWorkLogs);
		}
	...
	_throwAuthorizationError() {
		throw new CustomError('Staff MS authorization is required', INTERNAL_CODE);
	}
}
