...
import {
	...
	GLOBAL_SETTINGS_EXPIRATION_TIME,
} from './constant';
...
class Shift {
	...
	get hasStaffAuthorization() {
		const {settings} = Storage.get(STAFF_GLOBAL_SETTINGS) || {};
		const {hasStaffAuthorization = false} = settings || {};
	...
	async checkStaffMSAuthorization() {
		const {hasStaffAuthorization} = await this.getGlobalStaffSettings();
	 ...
	 * @param {number} settings.inactivityTimeout => time configureTimerd by the client to detect if the user is inactive or not
	 */

	async getGlobalStaffSettings() {
		try {
			Crashlytics.log('[getGlobalStaffSettings]:');
			const {
				isExpired,
				expirationTime = 0,
				settings = {},
			...
			const {result: globalSettings = {}} = await StaffService.getSetting('global');

			const {enabledShiftAndWorkLog = false, inactivityTimeout = 0, ...rest} = globalSettings || {};
			...
			Storage.set(STAFF_GLOBAL_SETTINGS, {
				...
				expirationTime: Date.now() + GLOBAL_SETTINGS_EXPIRATION_TIME,
				isExpired: false,
			});
			...
			Crashlytics.recordError(parsedError, 'Error getting global staff settings');
