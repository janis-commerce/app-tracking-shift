...
class Shift {
    ...
    async getUserOpenShift(params = {}) {
        ...
        try {
            ...
            const parsedShift = Boolean(openShift) ? openShift : undefined;
            return Promise.resolve(parsedShift);
        } catch (error) {
            return Promise.reject(error);
        }
    }

    async isUserShiftOpened(params = {}) {
        try {
            const openShift = await this.getUserOpenShift(params);
            return Promise.resolve(Boolean(openShift?.id));
        } catch(error) {
            return Promise.reject(error);
        }
    }
