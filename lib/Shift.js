...
class Shift {
	...
	async open({warehouseId = ''} = {}) {
		try {
			...
			const customError = CustomError.buildError(error);
	...
	async finish({date} = {}) {
		try {
			...
			const customError = CustomError.buildError(error);
			Crashlytics.recordError(customError, 'An error occurred while trying to close user shift');
			return Promise.reject(customError);
		}
	}
	...
	async openWorkLog(workLog = {}) {
		...
		try {
			...
			if (!ShiftWorklogs.isValidWorkLog(workLog)) {
				throw CustomError.buildError('must provide a valid activity to open a work log');
			}
			...
			const customError = CustomError.buildError(error);
			Crashlytics.recordError(customError, 'An error occurred while trying to open user workLog');
			return Promise.reject(customError);
		}
	}
	...
	async finishWorkLog(workLog = {}) {
		...
		try {
			...
			if (!ShiftWorklogs.isValidWorkLog(workLog)) {
				throw CustomError.buildError('must provide a valid activity to close a work log');
			}
			...
			if (!ShiftWorklogs.isValidWorkLog(currentWorkLog)) {
				throw CustomError.buildError('There is no active worklog to close');
			}
			...
			if (currentWorkLog?.startDate > finishWorkLog.endDate) {
				throw CustomError.buildError("The activity's end date is earlier than its start date.");
			}
			...
			const customError = CustomError.buildError(error);
			Crashlytics.recordError(customError, 'An error occurred while trying to close user workLog');
			return Promise.reject(customError);
		}
	}
	...
	async sendPendingWorkLogs() {
		try {
			...
			const customError = CustomError.buildError(error);
			Crashlytics.recordError(customError, 'Error posting pending work logs');
			return Promise.reject(customError);
		}
	}
	...
	async getUserOpenShift(params = {}) {
		try {
			...
			const customError = CustomError.buildError(error);
			Crashlytics.recordError(customError, 'Error getting open shift in staff service');
			return Promise.reject(customError);
		}
	}
	...
	async getWorkLogs(shiftId) {
		try {
			...
			if (!userShiftId) throw CustomError.buildError('Shift ID not found');
	...
	async reOpen({updateStatusAfterReOpen = true} = {}) {
		try {
			...
			if (this.isExpired()) {
				...
				throw CustomError.buildError('The deadline for ending the shift has been exceeded');
			}
			...
			const customError = CustomError.buildError(error);
			Crashlytics.recordError(customError, 'An error occurred while trying to re open user shift');
			return Promise.reject(customError);
		}
	}
	...
	async fetchWorklogTypes() {
		try {
			...
			const customError = CustomError.buildError(error);
			Crashlytics.recordError(customError, 'Error fetching worklog types from staff service');
			return Promise.reject(customError);
		}
	}
	...
	_throwAuthorizationError() {
		throw CustomError.buildError('Staff MS authorization is required');
	}
}
