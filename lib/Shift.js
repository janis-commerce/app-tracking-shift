...
class Shift {
	...
	get hasStaffAuthorization() {
		return getGlobalSettings().hasStaffAuthorization;
	}
	...
	async getGlobalStaffSettings() {
		try {
			const {result: settings = {}} = await StaffService.getSetting('global');

			const {enabledShiftAndWorkLog = false, inactivityTimeout = 0} = settings || {};
