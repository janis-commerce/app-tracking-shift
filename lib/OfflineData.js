...
class OfflineData {
	...
	save(id, data) {
		try {
			...
			Storage.set(OFFLINE_DATA, offlineData, {expireWithVersion: true});
		} catch (error) {
	...
	delete(id) {
		try {
			...
			Storage.set(OFFLINE_DATA, filteredData, {expireWithVersion: true});
		} catch (error) {
			throw new Error(error);
		}
	}
	...
	/**
	 * Replaces the whole offline data storage with the given records.
	 *
	...
	replaceAll(records = []) {
		try {
			Storage.set(OFFLINE_DATA, isArray(records) ? records : [], {expireWithVersion: true});
		} catch (error) {
			throw new Error(error);
		}
	}
