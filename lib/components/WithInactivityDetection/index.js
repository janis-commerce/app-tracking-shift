...
const WithInactivityDetection = (WrappedComponent) => {
	const InactivityWrapper = (props) => {
		...
		const startInactivityWorkLog = useCallback(() => {
			...
			const resetAt = ShiftInactivity.lastTimerResetAt;

			Shift.openWorkLog({
				...INTERNAL_WORKLOGS.INACTIVITY,
				...(isNumber(resetAt) &&
					resetAt > 0 && {startDate: new Date(resetAt + timeOut).toISOString()}),
		...
		useFocusEffect(
			useCallback(() => {
				...
				const elapsedTime = isNumber(persistedAt) ? Date.now() - persistedAt : 0;
