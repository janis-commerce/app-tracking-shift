...
const WithInactivityDetection = (WrappedComponent) => {
	const InactivityWrapper = (props) => {
		...
		const startInactivityWorkLog = useCallback(() => {
			...
			Shift.openWorkLog({...INTERNAL_WORKLOGS.INACTIVITY, startDate});
		}, [timeOut]);
		...
		useFocusEffect(
			useCallback(() => {
				...
				if (!!elapsedTime && elapsedTime >= timeOut) {
					startInactivityWorkLog(true, false);
					return () => {
						isFocusedRef.current = false;
					};
				}
				...
				ShiftInactivity.startTimer({
					...
					onTimeout: () => startInactivityWorkLog(true, false),
					ownerId: instanceId,
				});
				...
				if (previousState.match(/inactive|background/) && nextAppState === 'active') {
					...
					if (elapsedTime >= timeOut) {
						...
						ShiftInactivity.startTimer({
							...
							onTimeout: () => startInactivityWorkLog(false, true),
							ownerId: instanceId,
						});
					}
				}
