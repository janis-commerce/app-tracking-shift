...
import {generateRandomId, isNumber} from '@janiscommerce/apps-helpers';
...
const WithInactivityDetection = (WrappedComponent) => {
	const InactivityWrapper = (props) => {
		...
		const {
			...
			currentWorkLogId,
			isShiftInitializationDone = false,
		} = useShiftTracking();
		...
		const startInactivityWorkLog = useCallback(() => {
			if (!Shift.id || Shift.isExpired()) return;

			const expiresAt = ShiftInactivity.timerExpiresAt;
			const hasExpiresAt = isNumber(expiresAt) && expiresAt > 0;

			Shift.openWorkLog({
				...INTERNAL_WORKLOGS.INACTIVITY,
				// The inactivity can never start after the moment it is detected.
				...(hasExpiresAt && {
					startDate: new Date(Math.min(expiresAt, Date.now())).toISOString(),
				}),
		...
		useFocusEffect(
			useCallback(() => {
				...
				const expiresAt = ShiftInactivity.timerExpiresAt;
				const remainingTime = isNumber(expiresAt) ? expiresAt - Date.now() : timeOut;
				...
				// A resumed timer can never run longer than the configured timeout
				// (e.g. if the device clock moved backwards).
				ShiftInactivity.startTimer({
					duration: Math.min(remainingTime, timeOut),
		...
		}, [inactivityIsNotConfigured, shouldSkipDetection, isInactive]);
