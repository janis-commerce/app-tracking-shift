import {isString, isEmptyObject} from '@janiscommerce/apps-helpers';
import {isInternetReachable, Request, Crashlytics, errorParser} from './helpers';
import Storage from './db/StorageService';
import {SETTINGS_EXPIRATION_TIME, STAFF_SETTINGS} from './constant';

const SETTING_ENTITIES = ['global'];
class StaffApiServices {
	constructor() {
		this.service = 'staff';
	}

	/**
	 * Opens an user's shift
	 * @param {Object} params
	 * @param {string} params.warehouseId - shift current warehouse ID
	 * @returns {Promise<Object>} - Response from the API
	 */

	async openShift({warehouseId} = {}) {
		try {
			return await Request.post({
				service: this.service,
				namespace: 'shift-open',
				body: {
					...(isString(warehouseId) && !!warehouseId && {warehouseId}),
				},
			});
		} catch (error) {
			return Promise.reject(error);
		}
	}

	/**
	 * Updates the current user's shift
	 * @param {Object} params
	 * @param {string} params.warehouseId - shift current warehouse ID
	 * @returns {Promise<Object>} - Response from the API
	 */

	async updateShift({warehouseId} = {}) {
		try {
			return await Request.post({
				service: this.service,
				namespace: 'shift-update',
				body: {
					...(isString(warehouseId) && !!warehouseId && {warehouseId}),
				},
			});
		} catch (error) {
			return Promise.reject(error);
		}
	}
	/**
	 * Closes current user's shift
	 * @returns {Promise<Object>} - Response from the API
	 */

	async closeShift({endDate} = {}) {
		try {
			return await Request.post({
				service: this.service,
				namespace: 'shift-close',
				body: {
					...(isString(endDate) && !!endDate && {endDate}),
				},
			});
		} catch (error) {
			return Promise.reject(error);
		}
	}

	/**
	 * Posts work logs to the API
	 * @param {Object|Array<Object>} workLogs - Work log object or array of work log objects
	 * @param {string} workLogs.workLogTypeRefId - Reference ID of the work log type
	 * @param {Date} workLogs.startDate - Start date and time of the work log
	 * @param {Date} workLogs.endDate - End date and time of the work log
	 * @returns {Promise<Object>} - Response from the API
	 * @example
	 * // Single work log object
	 * postWorklog({
	 *   workLogTypeRefId: "123",
	 *   startDate: new Date("2024-01-01T09:00:00"),
	 *   endDate: new Date("2024-01-01T17:00:00")
	 * });
	 *
	 * // Array of work log objects
	 * postWorklog([
	 *   {
	 *     workLogTypeRefId: "123",
	 *     startDate: new Date("2024-01-01T09:00:00"),
	 *     endDate: new Date("2024-01-01T12:00:00")
	 *   },
	 *   {
	 *     workLogTypeRefId: "456",
	 *     startDate: new Date("2024-01-01T13:00:00"),
	 *     endDate: new Date("2024-01-01T17:00:00")
	 *   }
	 * ]);
	 */
	async postWorklog(workLogs) {
		try {
			if (!Array.isArray(workLogs)) {
				workLogs = [workLogs];
			}

			return await Request.post({
				service: this.service,
				namespace: 'work-log',
				body: [...workLogs],
			});
		} catch (error) {
			return Promise.reject(error);
		}
	}

	async getShiftsList(params) {
		const {filters, sort} = params || {};
		try {
			return await Request.list({
				service: this.service,
				namespace: 'shift',
				queryParams: {
					...(filters && {filters}),
					...(sort && {sort}),
				},
			});
		} catch (error) {
			return Promise.reject(error);
		}
	}

	async getWorkLogTypes() {
		try {
			return await Request.list({
				service: this.service,
				namespace: 'work-log-type',
				headers: {
					pageSize: 100,
				},
				queryParams: {
					filters: {
						status: 'active',
						isInternal: 'false',
					},
				},
			});
		} catch (error) {
			return Promise.reject(error);
		}
	}

	async getSettings() {
		try {
			const {isExpired, expirationTime = 0, settings = {}} = Storage.get(STAFF_SETTINGS) || {};

			const hasExpiredSettings = expirationTime <= Date.now() || isExpired;

			if (!hasExpiredSettings) return {...settings};

			const hasInternet = await isInternetReachable();

			if (!hasInternet && !isEmptyObject(settings)) return {...settings};

			const results = await Promise.all(
				SETTING_ENTITIES.map(async (entity) => {
					Crashlytics.log(`[getSettings]: fetching entity ${entity}`);
					const {result: entitySettings = {}} = await Request.get({
						service: this.service,
						namespace: 'setting',
						pathParams: [entity],
					});

					return entitySettings;
				})
			);

			const updatedSettings = Object.assign({}, ...results);

			Storage.set(STAFF_SETTINGS, {
				settings: updatedSettings,
				expirationTime: Date.now() + SETTINGS_EXPIRATION_TIME,
				isExpired: false,
			});

			return updatedSettings;
		} catch (error) {
			Storage.set(STAFF_SETTINGS, {
				settings: {},
				expirationTime: 0,
				isExpired: true,
			});

			const parsedError = errorParser(error);
			Crashlytics.recordError(parsedError, 'Error getting settings');
			return Promise.reject(parsedError);
		}
	}

	async getWorkLogsList(params) {
		const {filters, sort} = params || {};
		try {
			return await Request.list({
				service: this.service,
				namespace: 'work-log',
				queryParams: {
					...(filters && {filters}),
					...(sort && {sort}),
				},
			});
		} catch (error) {
			return Promise.reject(error);
		}
	}
}

export default new StaffApiServices();
