...
class ShiftInactivity {
	...
	startTimer({duration, onTimeout, instanceId} = {}) {
		...
		this.clearTimer();
		Storage.set(LAST_TIMER_RESET_AT, Date.now(), {expireWithVersion: true});
		this.isActive = true;
	...
	/**
	 * Reset the inactivity marker unconditionally. Used when the shift identity
	 * changes so a stale reset from a previous shift is not reused.
	 */
